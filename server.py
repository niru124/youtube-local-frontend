    ...
    while try_num <= 3:  # Try a given byte position three times
        ...
        if video:
            ...
            response, cleanup_func = util.fetch_url_response(
                url, send_headers, use_tor=use_tor, max_redirects=10
            )
        else:
        ...
        if first_attempt:
            start_response(
                str(response.status) + " " + response.reason, response_headers
            )

        content_length = int(dict(response_headers).get("Content-Length", 0))
        if response.status >= 400:
            print(
                'Error: Youtube returned "%d %s" while routing %s'
            ...
            if not content_part:
                ...
                if total_received < content_length:
                    if "Range" in send_headers:
                        int_range = parse_range(send_headers["Range"], content_length)
        ...
        print(
            "Error: Youtube closed the connection before",
            "providing all content. Retried three times:",
            url.split("?")[0],
        )
...
def site_dispatch(env, start_response):
    ...
    try:
        ...
        if "REQUEST_URI" in env:
            ...
            # decoding for us
            env["PATH_INFO"] = urllib.parse.unquote(env["REQUEST_URI"].split("?")[0])
        ...
        # redirect localhost to localhost/https://youtube.com
        if path == "" or path == "/":
            ...
            yield from yt_app(env, start_response)
            return
        elif path == "/history/search":
            env["SERVER_NAME"] = "youtube.com"  # Dummy server name for yt_app
            env["PATH_INFO"] = "/history/search"
            yield from yt_app(env, start_response)
            return
...
if __name__ == "__main__":
    if settings.allow_foreign_addresses:
        server = WSGIServer(
            ("0.0.0.0", settings.port_number), site_dispatch, log=FilteredRequestLog()
        )
    else:
        server = WSGIServer(
            ("127.0.0.1", settings.port_number), site_dispatch, log=FilteredRequestLog()
        )
