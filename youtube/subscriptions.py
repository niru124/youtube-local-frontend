    ...
    with open_database() as connection:
        with connection as cursor:
            channel_ids_to_check = [
                ...
                if not _is_subscribed(cursor, channel[0])
            ]

            rows = (
                (channel_id, channel_name, 0, 0)
                for channel_id, channel_name in channels
            )
            cursor.executemany(
                """INSERT OR IGNORE INTO subscribed_channels (yt_channel_id, channel_name, time_last_checked, next_check_time)
                                  VALUES (?, ?, ?, ?)""",
                rows,
            )
...
def _unsubscribe(cursor, channel_ids):
    ...
    cursor.executemany(
        "DELETE FROM subscribed_channels WHERE yt_channel_id=?",
...
def _get_videos(cursor, number_per_page, offset, tag=None):
    ...
    if tag is not None:
        db_videos = cursor.execute(
            """SELECT video_id, title, duration, time_published, is_time_published_exact, channel_name, yt_channel_id
                                      FROM videos
        ...
        videos.append(
            {
                ...
                "title": db_video[1],
                "duration": db_video[2],
                "time_published": exact_timestamp(db_video[3])
                if db_video[4]
                else posix_to_dumbed_down(db_video[3]),
                "author": db_video[5],
...
def _add_tags(cursor, channel_ids, tags):
    ...
    cursor.executemany(
        """INSERT OR IGNORE INTO tag_associations (tag, sql_channel_id)
...
def _get_all_tags(cursor):
    return [
        row[0]
        for row in cursor.execute("""SELECT DISTINCT tag FROM tag_associations""")
    ]
...
def _channels_with_tag(
    cursor, tag, order=False, exclude_muted=False, include_muted_status=False
):
    """returns list of (channel_id, channel_name)"""
...
def _schedule_checking(cursor, channel_id, next_check_time):
    cursor.execute(
        """UPDATE subscribed_channels SET next_check_time = ? WHERE yt_channel_id = ?""",
        [int(next_check_time), channel_id],
    )
...
def _is_muted(cursor, channel_id):
    return bool(
        cursor.execute(
            """SELECT muted FROM subscribed_channels WHERE yt_channel_id=?""",
            [channel_id],
        ).fetchone()[0]
    )
...
try:
    existing_thumbnails = set(
        os.path.splitext(name)[0] for name in os.listdir(thumbnails_directory)
    )
...
def autocheck_dispatcher():
    ...
    while True:
        if len(autocheck_jobs) == 0:
            ...
            earliest_job_index = min(
                range(0, len(autocheck_jobs)),
                key=lambda index: autocheck_jobs[index]["next_check_time"],
            ...
            if (
                time_until_earliest_job <= -5
            ):  # should not happen unless we're running extremely slow
                print(
                    "ERROR: autocheck_dispatcher got job scheduled in the past, skipping and rescheduling: "
                    + earliest_job["channel_id"]
                    + ", "
                    + earliest_job["channel_name"]
                    + ", "
                    + str(earliest_job["next_check_time"])
                )
                next_check_time = time.time() + 3600 * secrets.randbelow(60) / 60
                with_open_db(
                    _schedule_checking, earliest_job["channel_id"], next_check_time
                )
            ...
            if (
                time_until_earliest_job > 0
            ):  # it can become less than zero (in the past) when it's set to go off while the dispatcher is doing something else at that moment
                try:
                    new_job = autocheck_job_application.get(
                        timeout=time_until_earliest_job
                    )  # sleep for time_until_earliest_job time, but allow to be interrupted by new jobs
...
def start_autocheck_system():
    ...
    with open_database() as connection:
        with connection as cursor:
            now = time.time()
            for row in cursor.execute(
                """SELECT yt_channel_id, channel_name, next_check_time FROM subscribed_channels WHERE muted != 1"""
            ).fetchall():
                if row[2] is None:
                    next_check_time = 0
                else:
...
settings.add_setting_changed_hook("autocheck_subscriptions", autocheck_setting_changed)
if settings.autocheck_subscriptions:
    start_autocheck_system()
# ----------------------------
...
def _get_upstream_videos(channel_id):
    ...
    tasks = (
        ...
        gevent.spawn(_get_channel_videos_first_page, channel_id, channel_status_name),
    ...
    try:
        ...
        for entry in root:
            ...
            time_published = int(
                calendar.timegm(
                    time.strptime(
                        time_published_element.text, "%Y-%m-%dT%H:%M:%S+00:00"
                    )
                )
            )
    ...
    for i, video_item in enumerate(videos):
        if not video_item.get("description"):
            ...
            video_item["description"] = "".join(
                run.get("text", "") for run in video_item["description"]
            )
        ...
        if video_item["id"] in times_published:
            ...
            try:
                video_item["time_published"] = (
                    youtube_timestamp_to_posix(video_item["time_published"]) - i
                )  # subtract a few seconds off the videos so they will be in the right order
            except Exception:
                print(video_item)
        else:
            video_item["is_time_published_exact"] = False
    ...
    if len(videos) > 1:
        ...
        for i in range(len(videos) - 1):
            if (
                videos[i + 1]["time_published"] is None
    ...
    if len(videos) == 0:
        average_upload_period = (
            4 * 7 * 24 * 3600
        )  # assume 1 month for channel with no videos
    elif len(videos) < 5:
        average_upload_period = int(
            (time.time() - videos[len(videos) - 1]["time_published"]) / len(videos)
        )
    else:
        average_upload_period = int(
            (time.time() - videos[4]["time_published"]) / 5
        )  # equivalent to averaging the time between videos for the last 5 videos
    ...
    quantized_upload_period = (
        average_upload_period - (average_upload_period % (4 * 3600)) + 4 * 3600
    )  # round up to nearest 4 hours
    randomized_upload_period = quantized_upload_period * (
        1 + secrets.randbelow(50) / 50 * 0.5
    )  # randomly between 1x and 1.5x
    next_check_delay = (
        randomized_upload_period / 10
    )  # check at 10x the channel posting rate. might want to fine tune this number
    ...
    with open_database() as connection:
        with connection as cursor:
            ...
            is_first_check = cursor.execute(
                """SELECT time_last_checked FROM subscribed_channels WHERE yt_channel_id=?""",
                [channel_id],
            ...
            for i, video_item in enumerate(videos):
                ...
                v_id = video_item["id"]
                if (
                    existing_durs.get(v_id) is not None
            ...
            cursor.executemany(
                """INSERT OR IGNORE INTO videos (
                                  ...
                                  VALUES ((SELECT id FROM subscribed_channels WHERE yt_channel_id=?), ?, ?, ?, ?, ?, ?, ?)""",
                rows,
            )
            cursor.executemany(
                """UPDATE videos SET 
            ...
            cursor.execute(
                """UPDATE subscribed_channels
                              ...
                              WHERE yt_channel_id=?""",
                [int(time.time()), next_check_time, channel_id],
            )

            if settings.autocheck_subscriptions:
                if not _is_muted(cursor, channel_id):
                    autocheck_job_application.put(
                        {
...
def check_tags(tags):
    ...
    with open_database() as connection:
        with connection as cursor:
            for tag in tags:
                channel_id_name_list += _channels_with_tag(
                    cursor, tag, exclude_muted=True
                )
...
def check_specific_channels(channel_ids):
    with open_database() as connection:
        with connection as cursor:
            channel_id_name_list = []
            for channel_id in channel_ids:
                channel_id_name_list += cursor.execute(
                    """SELECT yt_channel_id, channel_name
                                                          FROM subscribed_channels
                                                          WHERE yt_channel_id=?""",
                    [channel_id],
                ).fetchall()
    ...
    if mime_type == "application/json":
            ...
            if "app_version_int" in info:  # NewPipe Format
                for item in info["subscriptions"]:
                    ...
                    if channel_id_match:
                        ...
                        print("WARNING: Could not find channel id in url", channel_url)
                        continue
        ...
        try:
            ...
            for outline_element in root[0][0]:
                if (outline_element.tag != "outline") or (
                    "xmlUrl" not in outline_element.attrib
                ):
                    continue

                channel_name = outline_element.attrib["text"]
                channel_rss_url = outline_element.attrib["xmlUrl"]
                channel_id = channel_rss_url[
                    channel_rss_url.find("channel_id=") + 11 :
                ].strip()
                channels.append((channel_id, channel_name))

        except (AssertionError, IndexError, defusedxml.ElementTree.ParseError) as e:
            return (
                "400 Bad Request: Unable to read opml xml file, or the file is not the expected format",
                400,
            )
        ...
        error += (
            " . Only subscription.json, subscriptions.csv files"
            " (from Google Takeouts)"
            " and XML OPML files exported from Youtube's"
            " subscription manager page are supported"
        )
        return (flask.render_template("error.html", error_message=error), 400)
        ...
        with connection as cursor:
            if group_by_tags:
                ...
                for tag in _get_all_tags(cursor):
                    sub_list = []
                    for channel_id, channel_name, muted in _channels_with_tag(
                        cursor, tag, order=True, include_muted_status=True
                    ):
                        sub_list.append(
                            {
                                "channel_url": util.URL_ORIGIN
                                + "/channel/"
                                + channel_id,
                                "channel_name": channel_name,
                                "channel_id": channel_id,
                                "muted": muted,
                                "tags": [
                                    t for t in _get_tags(cursor, channel_id) if t != tag
                                ],
                            }
                        )
    ...
    with open_database() as connection:
        with connection as cursor:
            if action == "add_tags":
                _add_tags(
                    cursor,
                    request.values.getlist("channel_ids"),
                    [
                        tag.lower()
                        for tag in list_from_comma_separated_tags(
                            request.values["tags"]
                        )
                    ],
                )
                ...
                unsubscribe_list = _get_channel_names(
                    cursor, request.values.getlist("channel_ids")
                )
                return flask.render_template(
                    "unsubscribe_verify.html", unsubscribe_list=unsubscribe_list
                )
                ...
                cursor.executemany(
                    """UPDATE subscribed_channels
                                      SET muted = 1
                                      WHERE yt_channel_id = ?""",
                    [(ci,) for ci in request.values.getlist("channel_ids")],
                )
            elif action == "unmute":
                cursor.executemany(
                    """UPDATE subscribed_channels
...
def get_subscriptions_page():
    ...
    with open_database() as connection:
        with connection as cursor:
            ...
            videos, number_of_videos_in_db = _get_videos(
                cursor, 60, (page - 1) * 60, tag
            )
...
def post_subscriptions_page():
    ...
    if action == "subscribe":
        if len(request.values.getlist("channel_id")) != len(
            request.values.getlist("channel_name")
        ):
            return (
                "400 Bad Request, length of channel_id != length of channel_name",
                400,
            )
        _subscribe(
            zip(
                request.values.getlist("channel_id"),
                request.values.getlist("channel_name"),
            )
        )
        return flask.jsonify(
            {"status": "success", "message": f"Subscribed to {channel_name}"}
        )

    elif action == "unsubscribe":
        with_open_db(_unsubscribe, request.values.getlist("channel_id"))
        return flask.jsonify(
            {"status": "success", "message": f"Unsubscribed from {channel_name}"}
        )
